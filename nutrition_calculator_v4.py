...
for time in slots:
    with st.expander(f"🍱 {time} 餐"):
        ...
        for i, row in df_nutrition.iterrows():
            with cols[i % 4]:
                val = st.number_input(f"{row['食材']} ({row['單位']}) - {time}", min_value=0.0, value=0.0, step=1)
...
st.markdown("## 📊 全日總攝取（與目標對照）")
st.markdown("#### 🎯 建議目標：熱量2000 kcal、蛋白質130g、碳水250g、油脂55g")
def color_text(val, name):
    tgt = {'熱量': 2000, '蛋白質': 130, '碳水': 250, '油脂': 55}
    return f"<span style='color:{'green' if val >= tgt[name] else 'red'}; font-weight:bold'>{val:.1f}</span> / {tgt[name]}"

st.markdown(f"熱量：{color_text(daily_total['熱量'], '熱量')} kcal", unsafe_allow_html=True)
st.markdown(f"蛋白質：{color_text(daily_total['蛋白質'], '蛋白質')} g", unsafe_allow_html=True)
st.markdown(f"碳水：{color_text(daily_total['碳水'], '碳水')} g", unsafe_allow_html=True)
st.markdown(f"油脂：{color_text(daily_total['油脂'], '油脂')} g", unsafe_allow_html=True)
