...
# 每日目標設定（依據日型）
daily_targets = {
    "訓練日": {"蛋白質": 115, "碳水": 250, "油脂": 55},
    "非訓練日": {"蛋白質": 110, "碳水": 200, "油脂": 55},
}
...
targets = daily_targets[day_type]
...
with col1:
    st.markdown(f"**熱量**<br>{daily_total['熱量']:.1f} kcal", unsafe_allow_html=True)
with col2:
    st.markdown(f"**蛋白質**<br>{daily_total['蛋白質']:.1f} / {targets['蛋白質']} g", unsafe_allow_html=True)
with col3:
    st.markdown(f"**碳水**<br>{daily_total['碳水']:.1f} / {targets['碳水']} g", unsafe_allow_html=True)
with col4:
    st.markdown(f"**油脂**<br>{daily_total['油脂']:.1f} / {targets['油脂']} g", unsafe_allow_html=True)
...
for i in range(temp_foods):
    with st.expander(f"🍡 臨時餐第 {i+1} 項"):
        st.text_input(f"食物名稱 {i+1}", key=f"name_{i}")
        cal = st.number_input("熱量 (kcal)", min_value=0.0, step=1.0, key=f"cal_{i}")
